...
if (isset($_POST['save'])) {
    ...
    $qry = "INSERT INTO organizations_monthly_breakdown (omb_id, oht_id, omb_month, omb_hours) VALUES (NULL, ?, ?, ?)";
  
    // Prepare the statement
    $stmt = $connection->prepare($qry);
  
    // Bind parameters
    $stmt->bind_param("iss", $ohtid, $ombm, $ombh);
...
<div class="max-w-md mx-auto bg-white rounded p-6 shadow-md flex flex-col items-center">
    <h2 class="text-2xl font-semibold mb-4">Add Organizations Monthly Breakdown</h2>
    <form action="#" method="POST" class="w-full">
        ...
        <div class="mb-4">
            ...
            <input type="text" id="day" name="ombm" class="mt-1 p-2 w-full border border-gray-300 rounded-md focus:outline-none focus:ring focus:ring-blue-200" required>
        </div>
        <div class="mb-4">
            <label for="hours" class="block text-sm font-medium text-gray-700">Organizations Monthly Breakdown Hours</label>
            <input type="text" id="hours" name="ombh" class="mt-1 p-2 w-full border border-gray-300 rounded-md focus:outline-none focus:ring focus:ring-blue-200" required>
        </div>
