...
if (isset($_GET['id'])) {
    ...
    $sql = "SELECT * FROM organizations_weekly_breakdown WHERE owb_id= '$id'";
...
if (isset($_POST['ohtid']) && isset($_POST['own']) && isset($_POST['owh']) && isset($_GET['id']))
{
    $id = $_GET['id'];
    $ohtid = $_POST['ohtid'];
    $own = $_POST['own'];
    ...
    // Prepare and execute the SQL query
    $sql = "UPDATE organizations_weekly_breakdown set oht_id= '$ohtid', owb_week_number= '$own', owb_hours= '$owh' where owb_id= '$id'";
...
<div class="max-w-md mx-auto bg-white rounded p-6 shadow-md flex flex-col items-center">
    ...
    <form action="#" method="post" class="w-full">
        ...
        <div class="mb-4">
            <label for="no" class="block text-sm font-medium text-gray-700">Organizations Weekly Breakdown Number</label>
            <input type="text" id="no" name="own" value="<?php echo ($own)?>" class="mt-1 p-2 w-full border border-gray-300 rounded-md focus:outline-none focus:ring focus:ring-blue-200" required>
        </div>
        <div class="mb-4">
            <label for="hours" class="block text-sm font-medium text-gray-700">Organizations Weekly Breakdown Hours</label>
            <input type="text" id="hours" name="owh" value="<?php echo ($owh)?>" class="mt-1 p-2 w-full border border-gray-300 rounded-md focus:outline-none focus:ring focus:ring-blue-200" required>
        </div>
